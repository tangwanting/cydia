...
function refreshAllInfo() {
    ...
    Object.keys(action.savedElements.placedElements).forEach(function(key) {
        ...
        if(key.substring(0, 3) === 'box'){
            ...
            // if (action.savedElements.placedElements[key]["musicplaying"] != null) {
    ...
    loopit: function(obj){
        ...
        */
        if(obj.initialTime){
            ...
            }else if (this.loopCount > 5 && this.loopCount < 8){
                loopSetTimeOutValue = 1000;
            }else if (this.loopCount > 8){
                loopSetTimeOutValue = obj.refreshTime;
            }
        }
...
function updateRSS(){
    ...
    var quote4Div = document.getElementById('quote4');
    var quote5Div = document.getElementById('quote5');

    if(quote1Div){
        if(action.savedElements.placedElements['quote1']['data-url']){
            action.getFeedQuote(action.quoteFeed, action.savedElements.placedElements['quote1']['data-url']);
        }
    }

    if(quote2Div){
        if(action.savedElements.placedElements['quote2']['data-url']){
            action.getFeedQuote(action.quoteFeed, action.savedElements.placedElements['quote2']['data-url']);
        }
    }

    if(quote3Div){
        if(action.savedElements.placedElements['quote3']['data-url']){
            action.getFeedQuote(action.quoteFeed, action.savedElements.placedElements['quote3']['data-url']);
        }
    }

    if(quote4Div){
        if(action.savedElements.placedElements['quote4']['data-url']){
            action.getFeedQuote(action.quoteFeed, action.savedElements.placedElements['quote4']['data-url']);
        }
    }

    if(quote5Div){
        if(action.savedElements.placedElements['quote5']['data-url']){
            action.getFeedQuote(action.quoteFeed, action.savedElements.placedElements['quote5']['data-url']);
        }
    }
}
...
var notShowingScreen = function(){
    ...
    screenOffTime = new Date().getTime();
    stopLoops();
};
