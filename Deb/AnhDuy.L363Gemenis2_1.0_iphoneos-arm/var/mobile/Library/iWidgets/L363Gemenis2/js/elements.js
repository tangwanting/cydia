...
weatherElements = {
    ...
    'lngwstring5': ["The current temperature is ", "temp", "&deg;, it's ", "conditionlowercase", " with a wind speed of ", "wind", ". </br> Your battery is at ", 'battery', '% and is ', 'charginglowercase', '.'],
...
systemElements = {
    ...
    'durationpie' : ['durationcircle'],
    'wifipie': ['wificircle'],
    'signalpie': ['signalcircle'],
...
weatherMethods = {
    ...
    getHoursUntilSunrise: function(){
        var sunrise = Number(this.cvtTwentyFour(injectedWeather.sunriseTime).split(':')[0]);
        var sunset = Number(this.cvtTwentyFour(injectedWeather.sunsetTime).split(':')[0]);
        var now = new Date();
        var hours = now.getHours();
        var sunObject = {};
        if(hours > sunrise){
            sunObject.time = sunset - hours;
            sunObject.isSunrise = false;
        }else{
            sunObject.time = sunrise - hours;
            sunObject.isSunrise = true;
        }
        return sunObject;
    },
    timeuntilsunrise: function(){
        if(weatherMethods.getHoursUntilSunrise().isSunrise){
            return weatherMethods.getHoursUntilSunrise().time + "hrs until sunrise";
        }else{
            return weatherMethods.getHoursUntilSunrise().time + "hrs until sunset";
        }
    },
    updated: function() {
...
systemMethods = {
    ...
    wificircle: function (){ //main.js
        try{
            if(wifiProgress){
                if(wifiProgress.value != this.percent[Number(injectedSystem.wifiBars)]){
                    wifiProgress.value = this.percent[Number(injectedSystem.wifiBars)];
                }
                if(injectedSystem.wifiName === "(null)"){
    ...
    signalcircle: function (){ //main.js
        try{
            if(signalProgress){
                var signalValue = null;
                if(injectedSystem.signalStrength > 6){
                    signalValue =  injectedSystem.signalStrength + "%";
                }else{
                    signalValue =  this.signalArray[injectedSystem.signalBars];
                }

                if(signalProgress.value != signalValue){
                    signalProgress.value = signalValue;
                }
            }
        }catch(err){}
        return "";
    ...
    combinedbadge: function(){
        return injectedSystem.allbadges;
    },
    mailbadge: function(){
